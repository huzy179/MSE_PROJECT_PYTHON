...
const ExamList: React.FC<ExamListProps> = ({
  ...
  const fetchExams = async () => {
    try {
      ...
      setError(err.response?.data?.detail || 'Có lỗi xảy ra khi tải danh sách đề thi');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white shadow-sm rounded-lg border overflow-hidden">
        {exams.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">Không có đề thi</h3>
            <p className="mt-1 text-sm text-gray-500">Bắt đầu bằng cách tạo đề thi mới.</p>
          </div>
        ) : (
                  ...
                  <tr key={exam.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        {(user?.role === USER_ROLES.ADMIN || exam.created_by === user?.id) && (
                          <button
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            {Math.min(currentPage * (filters.page_size || 10), totalCount)} trong tổng số {totalCount} đề thi
          </div>
