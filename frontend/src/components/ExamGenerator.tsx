...
const ExamGenerator: React.FC<ExamGeneratorProps> = ({ onGenerate, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.total_questions > availableCount) {
      setError(`Không đủ câu hỏi cho môn ${formData.subject}. Chỉ có ${availableCount} câu hỏi có sẵn.`);
      setLoading(false);
      return;
    }
  ...
  const isValidQuestionCount = formData.total_questions <= selectedSubjectQuestions;

  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-900">Sinh đề thi tự động</h2>
        <button
          onClick={onCancel}
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-blue-700">
              <p>Hệ thống sẽ tự động chọn ngẫu nhiên câu hỏi từ ngân hàng câu hỏi theo môn học đã chọn.</p>
              <p>Thứ tự các lựa chọn A, B, C, D sẽ được xáo trộn ngẫu nhiên cho mỗi câu hỏi.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            />
            {formData.subject && (
              <p className={`mt-1 text-sm ${isValidQuestionCount ? 'text-gray-500' : 'text-red-600'}`}>
                Có sẵn: {selectedSubjectQuestions} câu hỏi cho môn {formData.subject}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center">
          ...
          />
          <label htmlFor="shuffle_choices" className="ml-2 block text-sm text-gray-900">
