...
class DocumentParser:
    ...
    def __init__(self, file_path: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Cannot read document: {str(e)}"
            )
    ...
    def parse_question_row(
        self, row_text: List[str], current_q: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class QuestionService:
    ...
    @staticmethod
    def import_data(
        list_data: List[Dict[str, Any]], db: Optional[Session] = None
    ) -> Dict[str, Any]:
        ...
        try:
            questions = [
                QuestionService.create_question_from_dict(item) for item in list_data
            ]
            ...
            session.rollback()
            return {"code": 500, "message": f"Import failed: {str(e)}"}
        finally:
            if should_close:
                session.close()
    ...
    @staticmethod
    def get_questions(
        skip: int = 0, limit: int = 100, db: Optional[Session] = None
    ) -> List[Question]:
...
def get_question_by_id(db: Session, question_id: int) -> Optional[Question]:
    ...
    return (
        db.query(Question)
        .filter(and_(Question.id == question_id, Question.deleted_at.is_(None)))
        .first()
    )
...
def update_question(
    db: Session, question_id: int, question: QuestionUpdate, user_id: int
) -> Optional[Question]:
    """Update a question"""
...
def delete_question(db: Session, question_id: int) -> bool:
    ...
    db_question.deleted_at = func.now()
    db.commit()
    return True
...
def get_subjects(db: Session) -> List[str]:
    ...
    subjects = (
        ...
        .filter(and_(Question.deleted_at.is_(None), Question.subject.isnot(None)))
        .distinct()
        .all()
    )
